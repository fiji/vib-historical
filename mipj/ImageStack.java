...
public class ImageStack
{
    ...
    public ImageStack( ij.ImageStack ijs ) throws ClassCastException
    {
    ...
    public ImageStack( String fileOrDirectory, float scale ) throws Exception
    {
        if (scale < 0.0f)
            throw new Exception("ImageStack: A negative scale doesn't make sense!");
        openScale = scale;
        loadFiles( fileOrDirectory );
    }

    public ImageStack( String fileOrDirectory, int rtsize ) throws Exception
    {
        if (rtsize != 256)
    ...
    public ImageStack( String fileOrDirectory ) throws Exception
    {
    ...
    /** Resizes all the images in the stack by the given factor. It uses ImageJ
    to perform this. */

    public void scale( float scaleby )
    {

        if ( scaleby <= 0.0f) return;
